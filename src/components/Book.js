...
export const Book = ({ book, handleOnClick }) => {
  ...
  const handleOnRemove = () => {
    dispatch(actions.deleteBook(book.id));
  };
  ...
  return (
    ...
    >
      <div className="w-11/12 flex flex-col border-r" onClick={handleOnClick}>
      ...
      <div className="w-1/12" onClick={handleOnRemove}>
        <div className="flex flex-col items-center">
          <TrashCanIcon className="text-sky-600 hover:scale-105 hover:text-sky-800" />
        </div>
      </div>
    </article>
  );
